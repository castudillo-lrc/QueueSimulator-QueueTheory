...
(function () {
  function require(name) {
    ...
    var styles = ["webkitTransform", "MozTransform", "msTransform", "OTransform", "transform"];var el = document.createElement("p");var style;for (var i = 0; i < styles.length; i++) {
      style = styles[i];if (null != el.style[style]) {
        module.exports = style;break;
      }
    }
  });require.register("component~has-translate3d@0.0.3", function (exports, module) {
    var prop = require("component~transform-property@0.0.1");if (!prop || !window.getComputedStyle) {
    ...
    }var styl = document.body.style;var bool = "transition" in styl || "webkitTransition" in styl || "MozTransition" in styl || "msTransition" in styl;
    ...
    var events = require("component~event@0.1.4");var watch = ["transitionend", "webkitTransitionEnd", "oTransitionEnd", "MSTransitionEnd", "animationend", "webkitAnimationEnd", "oAnimationEnd", "MSAnimationEnd"];module.exports = CssEmitter;function CssEmitter(element) {
      ...
      var self = this;function on() {
        self.unbind(on);fn.apply(self.el, arguments);
      }self.bind(on);return this;
    };
  });require.register("component~once@0.0.1", function (exports, module) {
    var n = 0;var global = function () {
      return this;
    }();module.exports = function (fn) {
      var id = n++;function once() {
        if (this == global) {
          if (once.called) return;once.called = true;return fn.apply(this, arguments);
    ...
    var has = require("yields~has-transitions@1.0.0"),
        ...
        once = require("component~once@0.0.1");var supported = has();module.exports = after;function after(el, fn) {
      if (!supported || !has(el)) return fn();emitter(el).bind(fn);return fn;
    }after.once = function (el, _fn) {
      var callback = once(_fn);after(el, _fn = function fn() {
        emitter(el).unbind(_fn);callback();
      });
    };
    ...
    var Emitter = require("component~emitter@1.2.0");var query = require("component~query@0.0.3");var after = require("yields~after-transition@0.0.1");var has3d = require("component~has-translate3d@0.0.3");var ease = require("yields~css-ease@0.0.1");var translate = has3d ? ["translate3d(", ", 0)"] : ["translate(", ")"];module.exports = Move;var style = window.getComputedStyle || window.currentStyle;Move.version = "0.5.0";Move.ease = ease;Move.defaults = { duration: 500 };Move.select = function (selector) {
    ...
    }Emitter(Move.prototype);Move.prototype.transform = function (transform) {
      ...
      n = this._duration = "string" == typeof n ? parseFloat(n) * 1e3 : n;return this.setVendorProperty("transition-duration", n + "ms");
    };Move.prototype.delay = function (n) {
      n = "string" == typeof n ? parseFloat(n) * 1e3 : n;return this.setVendorProperty("transition-delay", n + "ms");
      ...
      if (!style) return;var self = this;return this.on("start", function () {
        var curr = parseInt(self.current(prop), 10);self.set(prop, curr + val + "px");
      });
      ...
      if (fn instanceof Move) {
        ...
        var clone = new Move(this.el);clone._transforms = this._transforms.slice(0);this.then(clone);clone.parent = this;return clone;
      }return this;
      ...
      var self = this;this.emit("start");if (this._transforms.length) {
        this.setVendorProperty("transform", this._transforms.join(" "));
      }this.setVendorProperty("transition-properties", this._transitionProps.join(", "));this.applyProperties();if (fn) this.then(fn);after.once(this.el, function () {
        self.reset();self.emit("end");
      });return this;
    };
